...
public class Unit : MonoBehaviour {
    ...
    public enum Phases { MOVE, ATTACK, WAIT };
    private Phases phase = Phases.MOVE;
    public int PLAYER;
    public int MAX_HP;
    private int hp;
    ...
    public enum Statuses { Moving, Attacking, Defending, Idle };
    ...
    public void move(HexPosition[] path) {
        ...
        t = 0;
        n = 0;
        this.status = Unit.Statuses.Moving;
    ...
    private GameObject showDamage(int damage) {
        return TextFloaterBehaviour.Spawn(damage.ToString(), this.DamagingPosition(), 1400, Color.red, 4f);
    }

    private Vector3 DamagingPosition() {
        var cube = gameObject.transform.Find("Unit Model");
    ...
    public void defend(float strength, float variation) {
        ...
        this.grid.wait();
        this.status = Unit.Statuses.Defending;
        this.hp -= damage;
        ...
        this.status = Unit.Statuses.Idle;
    }
    ...
    void UpdateMove() {
        ...
        if (this.status != Unit.Statuses.Moving) {
            return;
        }
        if (path.Length == 2) {
            if (t >= 1) {
