...
public class HexGrid : MonoBehaviour {
    ...
    private bool selectSelectable() {
        ...
        foreach (Unit otherUnit in units) {
            if (otherUnit.PLAYER == player && otherUnit.Phase != Unit.Phases.WAIT) {
    ...
    void Start() {
        this.SpawnUnit<Fighter>(0, new HexPosition(1, 1));
        this.SpawnUnit<Shooter>(0, new HexPosition(2, 2));
        this.SpawnUnit<Fighter>(1, new HexPosition(-1, -1));
        this.SpawnUnit<Shooter>(1, new HexPosition(-2, -2));
        //timeout = MAX_TIME;
    ...
    private void select() {
        if (mouse.isSelected("Selectable")) {
            ...
            switch (unit.Phase) {
                case Unit.Phases.MOVE:
                    turn = Turn.MOVE;
                    break;
                case Unit.Phases.ATTACK:
                    turn = Turn.ATTACK;
                    break;
                default:
                    print("Error: Action " + ((Unit)mouse.getValue("Unit")).Phase + " not implemented.");
                    break;
            }
        }
    }
    ...
    void OnGUI() {
        if (!modeSelected) {
            ...
            if (GUI.Button(new Rect(10, 100, 90, 20), "Add unit 2")) {
                SpawnUnit<Fighter>(0, new HexPosition(2, 2));
                SpawnUnit<Fighter>(0, new HexPosition(2, 3));
                SpawnUnit<Fighter>(0, new HexPosition(2, 4));
                return;
            }
            return;
        }
